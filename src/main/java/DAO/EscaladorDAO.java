...
public class EscaladorDAO implements GenericDAO<Escalador> {
    ...
    public void modificar(Escalador e) {
        String sql = "UPDATE Escaladors SET nom = ?, edat = ?, nivell_maxim = ?, nacionalitat = ? WHERE alias = ?";
        ...
        try (Connection con = gestorConexio.getConexion();
            ...
            pstmt.setString(3, e.getNivellMaxim());
            pstmt.setString(4, e.getNacionalitat());
            pstmt.setString(5, e.getAlias());

            int filasAfectadas = pstmt.executeUpdate();

            if (filasAfectadas > 0) {
                System.out.println("Dades de l'escalador actualitzades correctament!");
            } else {
                System.out.println("No s'ha trobat cap escalador amb l'alias: " + e.getAlias());
            }
